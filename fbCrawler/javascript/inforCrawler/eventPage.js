...
function runMoreScrape(_id,defaultUrl) {
    $.ajax({
        ...
        var defaultWorkEduName=[];
		var defaultWorkEduId=[];
        _doc.find('.experience').each(function() {
			var valueName=$(this).children().eq(1).children().children().children().text();
			if ($.inArray(valueName, defaultWorkEduName) == -1)
			{
				defaultWorkEduName.push(valueName);
			}       
			var valueId=$(this).children().eq(1).children().children().children().attr("href");
			
			if (valueId.indexOf("profile")>0)
			{	
				valueId=valueId.substring(16,valueId.length);
			}
			else
			{	
				valueId=valueId.substring(1,valueId.length);
			}
			if ($.inArray(valueId, defaultWorkEduId) == -1)
			{
				defaultWorkEduId.push(valueId);
			}
        ...
        _doc.find('div[title="Facebook"]').each(function () {
			...
			defaultFacebook=defaultFacebook.substring(1,defaultFacebook.length);
        });
