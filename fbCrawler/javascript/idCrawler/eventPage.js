...
function runScrape(_id) {
    $.ajax({
		...
		var defaultUser;
		var defaultId;
		defaultUser=_docRoot.filter('noscript').eq(0).text();
		var n = defaultUser.indexOf("URL=/");
		var m;
		if (defaultUser.indexOf("profile")>0)
			{	
				//number
				n+=20;
				m=defaultUser.indexOf("&");
				defaultId=defaultUser.substring(n,m);
				defaultUser="";
			}
		else
			{	
				...
				n+=5;
				m=defaultUser.indexOf("?");
				defaultUser=defaultUser.substring(n,m);
				_docRoot.find("#pagelet_timeline_main_column").each(function(){
					defaultId=$(this).attr("data-gt");
					defaultId=defaultId.substring(defaultId.indexOf(":")+2,defaultId.indexOf(",")-1);
				});
			}
			
		lstResult[lstResult.length] = {'user':defaultUser,'id':defaultId};
		nextScrape();
		//runMoreScrape(_id);
    }).fail(function () {
        nextScrape();
...
function runMoreScrape(_id) {
    $.ajax({
        ...
        lstResult[lstResult.length] = {'id':_id,'workEdu':defaultWorkEdu,'place':defaultPlace, 'phone': defaultPhone,'address':defaultAddress,'facebook':defaultFacebook, 'email': defaultEmail,'birthday':defaultBirthday, 'gender':defaultGender,'interestedIn':defaultInterestedIn,'languages':defaultLanguages,'languages':defaultLanguages,'religiousViews':defaultReligiousViews,'	':defaultNickname,'relationship':defaultRelationship,'family':defaultFamily,'quote':defaultQuote };
        nextScrape();
    }).fail(function () {
        nextScrape();
    });
